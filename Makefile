...
####################################
# INIZIO MODIFICA PER USARE LA CACHE DEI SORGENTI DI TEXLIVE EXPAT E FONTCONFIG
###################################
local/source/texlive.txt:
	mkdir -p source
	tar -xzf texlive-source-cache/texlive-2023.0.tar.gz -C source --strip-components=1
	find source > source/texlive.txt

local/source/expat.txt:
	mkdir -p source
	tar -xzf texlive-source-cache/expat-2.5.0.tar.gz -C source --strip-components=1
	find source > source/expat.txt

local/source/fontconfig.txt:
	mkdir -p source
	tar -xzf texlive-source-cache/fontconfig-2.13.96.tar.gz -C source --strip-components=1
	find source > source/fontconfig.txt
####################################
# FINE MODIFICA PER USARE LA CACHE DEI SORGENTI DI TEXLIVE EXPAT E FONTCONFIG
###################################
...
####################################
# INIZIO MODIFICA PER USARE LA CACHE DELLA ISO DI TEXLIVE
###################################
source/texmfrepo.txt:
	mkdir -p $(basename $@)
	cat texlive-iso-cache/texlive2023-20230313.iso.* | bsdtar -x -C $(basename $@)
	find $(basename $@) > $@
####################################
# FINE MODIFICA PER USARE LA CACHE DELLA ISO DI TEXLIVE
###################################
...
build/wasm/ubuntu/%.js: $(TEXMFFULL)
	mkdir -p $(dir $@)
	$(PYTHON) $(EMROOT)/tools/file_packager.py $(basename $@).data --js-output=$@ --export-name=BusytexPipeline --lz4 --use-preload-cache $(shell $(PYTHON) ubuntu_package_preload.py --package $(subst _, ,$(notdir $(basename $@))) --texmf $(TEXMFFULL) --path ubuntu-texlive-package-files --skip-log $@.skip.txt --good-log $@.good.txt --providespackage-log $@.providespackage.txt --ubuntu-log $@.ubuntu.txt)
	-cat $@.providespackage.txt $@ > $@.tmp; mv $@.tmp $@
##############################################
...
.PHONY: ubuntu-wasm
ubuntu-wasm: build/wasm/ubuntu/texlive-latex-extra.js build/wasm/ubuntu/texlive-base_texlive-latex-base_texlive-latex-recommended_texlive-science_texlive-fonts-recommended_texlive-pictures_texlive-publishers_texlive-humanities.js
