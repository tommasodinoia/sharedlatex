...
/**
 * Initializes the SyncTeX object by decompressing and parsing the provided data.
 * @param {Uint8Array} synctex - The compressed SyncTeX data.
 * @param {Function} parseSyncTex - The function to parse the decompressed SyncTeX data.
 * @returns {Object|null} - The initialized SyncTeX object or null if an error occurs.
 */
export function initializeSynctex(synctex, parseSyncTex) {
  try {
    const decompressedData = pako.ungzip(synctex, { to: "string" });
    const pdfSyncObject = parseSyncTex(decompressedData);

    if (!pdfSyncObject) {
      console.error("[syncTeXManager] Error parsing decompressed SyncTeX file.");
      return null;
    }

    console.log("[syncTeXManager] SyncTeX data parsed. Creating hBlocks index by page...");
    pdfSyncObject.hBlocksByPage = {};

    if (pdfSyncObject.hBlocks && Array.isArray(pdfSyncObject.hBlocks)) {
      pdfSyncObject.hBlocks.forEach(block => {
        const pageNum = block.page;
        if (!pageNum) return;
        if (!pdfSyncObject.hBlocksByPage[pageNum]) {
          pdfSyncObject.hBlocksByPage[pageNum] = [];
        }
        pdfSyncObject.hBlocksByPage[pageNum].push(block);
      });
      console.log("[syncTeXManager] hBlocks index by page created.", pdfSyncObject.hBlocksByPage);
    } else {
      console.warn("[syncTeXManager] pdfSyncObject.hBlocks not found or is not an array, unable to create index.");
    }

    return pdfSyncObject;
  } catch (err) {
    console.error("[syncTeXManager] Error decompressing or parsing SyncTeX:", err);
    return null;
  }
}
...
/**
 * Handles the logic for reverse SyncTeX navigation.
 * @param {Object} pdfSyncObject - The SyncTeX data object.
 * @param {Array} pdfCoords - [pdfX, pdfY] coordinates.
 * @param {number} page - The PDF page number.
 * @param {number} pageHeight - The height of the PDF page.
 */
export async function manageReverseSyncTex(pdfSyncObject, pdfCoords, page, pageHeight) {
  if (!pdfSyncObject) {
    console.warn("[compileManager] SyncTeX data (pdfSyncObject) not yet available.");
    return;
  }

  if (!texEditor) {
    console.error("[compileManager] texEditor not initialized.");
    return;
  }

  try {
    const result = await reverseSyncTeX(pdfSyncObject, pdfCoords, page, pageHeight);
    if (result && result.lineNumber) {
      console.log(`[compileManager] Reverse SyncTeX result: Line ${result.lineNumber}, File: ${result.sourceFilePath}`);
      texEditor.revealLineInCenter(result.lineNumber);
      texEditor.setPosition({ lineNumber: result.lineNumber, column: 1 });
      texEditor.focus();
    } else {
      console.warn("[compileManager] Reverse SyncTeX did not produce a valid result.");
    }
  } catch (error) {
    console.error("[compileManager] Error during reverseSyncTeX execution:", error);
  }
}