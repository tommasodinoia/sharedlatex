...
/**
 * BusytexDataPackageResolver is a class designed to resolve LaTeX package dependencies
...
class BusytexPipeline
{
    ...
    constructor(busytex_js, busytex_wasm, data_packages_js, preload_data_packages_js, texmf_local, print, on_initialized, preload, script_loader)
    {
        ...
        /*
        * Qui imposto la directory che corrisponde al mio progetto /home/web_user/project_dir
        */
    ...
    async reload_module(env, project_dir, data_packages_js = [], report_applet_versions = false)
    {
        ...
        const Module =
        {
            ...
            /*
            * Con questa funzione carico il modulo WASM
            * OSSERVAZIONI: il modulo WASM dovremmo caricarlo in memoria nel file main.js
            * in modo da poter sincronizzare il filesytem di WASM con i file di progetto.
            */
    ...
    async compile(files, main_tex_path, bibtex, verbose, driver, data_packages_js = [])
    {
      ...
      //  const xetex     = ['xelatex' ,   '--no-shell-escape', '--interaction=batchmode', '--halt-on-error', '--no-pdf'           , '--fmt', this.fmt.xetex , tex_path].concat((this.verbose_args[verbose] || this.verbose_args[BusytexPipeline.VerboseSilent]).xetex);
       ...
       /*
       * - Qui viene montato un filesystem di tipo MEMFS, che è un filesystem in memoria, 
       * e viene montato nella directory del progetto.
       * - I file e le directory presenti in files vengono scritti nel filesystem MEMFS.
       * - Viene cambiata la directory di lavoro in source_dir.
       *
       * OSSERVAZIONI: 
       * - ad ogni compilazione TUTTI i file di progetto vengono sovrascritti
       * nel filesystem.
       * - verificare come usare un fylesystem persistente come IDBFS
       * 
       */
...
/*
            ...
            console.log(FS.analyzePath(log_path).exists);

            if (is_bibtex && !(FS.analyzePath(aux_path).exists)) {
                console.log(`Il file ${aux_path} non esiste nella directory corrente.`);
        ...
        const readDirectoryRecursive = (FS, PATH, path) => {
            ...
            for (const entry of entries) {
                ...
                if (FS.isDir(stats.mode)) {
                    result.push({ type: 'directory', name: fullPath });
                    result = result.concat(readDirectoryRecursive(FS, PATH, fullPath));
                } else if (FS.isFile(stats.mode)) {
                    result.push({ type: 'file', name: fullPath });
                }
            }
